...
class CrtlWorld(nn.Module):
    def __init__(self, args):
        ...
        unet = UNetSpatioTemporalConditionModel()   #! CtrlWorld仅在SVD的UNet的forward方法中做了极小的修改，不影响模型权重加载
    ...
    def configure_trainable_params(self):
        if self._trainable_configured:
            return

        if getattr(self.args, "use_lora", False):
            self._enable_unet_lora()
        else:
            self.unet.requires_grad_(True)

        self.pipeline.unet = self.unet
        self._trainable_configured = True

    def _enable_unet_lora(self):
        try:
        ...
        target_modules = list(getattr(self.args, "lora_target_modules", ("to_q", "to_k", "to_v", "to_out.0")))
