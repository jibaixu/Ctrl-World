...
class WanVACEPipeline(DiffusionPipeline, WanLoraLoaderMixin):
    ...
    @torch.no_grad()
    def construct_latents_from_video(
        self,
        ...
        """
        Given a video(List[PIL]), need to finish i2v task.
        - Firstly, construct reference video with first frame only;
        - Secondly, construct mask;
        - Finally, extract latents and condition latents.
        """
        assert len(video) > 1
    ...
    def __call__(
        ...
        timesteps = self.scheduler.timesteps.to(dtype=torch.long, device=device)
        ...
        latents = self.prepare_latents(
            ...
            height,
            width,
            num_frames + num_reference_images * self.vae_scale_factor_temporal, #! 被扩散的噪声latent在时间维度上需要包含参考帧
            torch.float32,
            device,
            generator,
            latents,
        ...
        if not output_type == "latent":
            ...
            #! 转换数值精度
            latents = latents.to(vae_dtype) #! transformer 采用 bfloat16，输入 vae 时直接类型转换为 float32 即可
            #! 计算VAE解码空间的均值和标准差
            latents_mean = (
